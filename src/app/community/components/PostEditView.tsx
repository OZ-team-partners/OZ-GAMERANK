...
import type { Category, PostFormData, CommunityPost } from '../types';
...
export default function PostEditView({ post, user, onBack, onSubmit }: PostEditViewProps) {
  const [formData, setFormData] = useState<PostFormData>({
    title: post.title || '',
    content: post.content || '',
    category: post.category || '온라인게임'
  });
      ...
      <div className="space-y-6">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700 p-12 text-center">
          <div className="text-slate-400">
            <p className="text-lg">수정 권한이 없습니다</p>
            <p className="text-sm mt-2">본인이 작성한 게시글만 수정할 수 있습니다.</p>
          </div>
        </div>
      </div>
    );
