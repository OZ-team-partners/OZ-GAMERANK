...
export default function SectionPage() {
  ...
  // Supabase에서 Android 게임 순위 데이터 가져오기
  useEffect(() => {
    const fetchAndroidRankings = async () => {
      try {
        setLoading(true);
        const data = await getGameRankings("mobile_android", 100);
        setRankings(data);
      } catch (err) {
        setError("Android 게임 순위 데이터를 불러오는 중 오류가 발생했습니다.");
        console.error("Android 순위 데이터 가져오기 실패:", err);
      } finally {
        setLoading(false);
      }
    };
        ...
        <main className="flex-1 p-5">
          ...
          <div className="mb-6">
            ...
            <p className="text-slate-400">
              Google Play Store에서 가장 인기 있는 Android 게임 순위입니다.
            </p>
