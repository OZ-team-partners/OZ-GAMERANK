...
export default function SectionPage() {
  ...
  useEffect(() => {
    const fetchSteamRankings = async () => {
      try {
        setLoading(true);
        const response = await fetch("/api/steam-ranking");
        const result: SteamRankingResponse = await response.json();
        ...
        setError("Steam 랭킹 데이터를 불러오는 중 오류가 발생했습니다.");
        console.error("Steam 랭킹 가져오기 실패:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <main className="flex-1 p-5">
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-red-400">Steam 게임 순위를 불러오는 중...</p>
              </div>
            </div>
          </main>
        </div>
